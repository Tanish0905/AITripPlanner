...
//   // Log the TripData to inspect its structure
...
//   //   // Get user from localStorage and check if it exists
//   //   const user = JSON.parse(localStorage.getItem('user'));
//   //   if (!user) {
//   //     setLoading(false);
//   //     alert("User not found, please log in.");
...
//   //   // Generate a document ID (using current timestamp)
//   //   const docId = Date.now().toString();

//   //   try {
//   //     // Save the actual TripData to Firestore under the 'AITrips' collection
//   //     await setDoc(doc(db, 'AITrips', docId), {
//   //       ...TripData,  // Save the TripData passed to the function
//   //       createdBy: user.id,  // Optionally, you can add the user info if needed
//   //       timestamp: new Date()  // Adding a timestamp field (optional)
//   //     });
//   //     alert("Trip saved successfully!");
//   //   } catch (error) {
//   //     console.error("Error saving trip:", error);
...
//         console.log('response', res);
//         localStorage.setItem('user', JSON.stringify(res.data));
//         localStorage.setItem('access_token', tokenInfo?.access_token);  // Saving the access token
//         setOpenDialog(false);
//         OnGenerateTrip();
//       })
//       .catch((error) => {
